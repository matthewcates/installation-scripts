...
then
  echo "Adding PPA for up-to-date Node.js runtime. Give your password if asked."
...
echo "Updates packages. May ask for your password."
sudo apt-get update -y

echo "Installs packages. Give your password if asked."
...
echo "Installs RVM (Ruby Version Manager) for handling the Ruby installation"
...
echo "Installs Rails and the bundler (gem: --no-ri --no-rdoc)" > ~/.gemrc
gem install bundler
gem install rails
...
It is just a hiccup with the shell. You can correct this issue with the command below:                                   
    $ source ~/.rvm/scripts/rvm                                                  
